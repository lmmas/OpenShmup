...
final public class Menus {
    ...
    public static class Screens {
        ...
        private static MenuScreen PopupScreen() {
            ...
            ActionButton closeButton = ActionButtons.RoundedRectangleButton(1, closeButtonSize, new Vec2D(1800, 1000), menuButtonRoundingRadius, menuButtonBorderWidth, Color.menuButtonColor, Color.menuButtonBorderColor, "Close", menuButtonLabelStyle, () -> Engine.getCurrentMenu().removeMenuScreen(popupMenu));
            ...
            for (int i = 0; i < Editor.getLoadedGames().size(); i++) {
                ...
                popupMenu.addItem(ActionButtons.RoundedRectangleButton(1, buttonSize, new Vec2D(resolution.x / 2, 800 - (buttonSize.y + 20f) * i), menuButtonRoundingRadius, menuButtonBorderWidth, Color.menuButtonColor, Color.menuButtonBorderColor, game.getGameName(), menuButtonLabelStyle, onclick));
            }
            return popupMenu;
        }
    }
    ...
    public static Menu MainMenu() {
        ...
        ActionButton button1 = ActionButtons.RoundedRectangleButton(1, buttonSize, new Vec2D(resolution.x / 2, 500), menuButtonRoundingRadius, menuButtonBorderWidth, Color.menuButtonColor, Color.menuButtonBorderColor, "Edit game", menuButtonLabelStyle, editGameAction);
        ActionButton button2 = ActionButtons.RoundedRectangleButton(1, buttonSize, new Vec2D(resolution.x / 2, 300), menuButtonRoundingRadius, menuButtonBorderWidth, Color.menuButtonColor, Color.menuButtonBorderColor, "Quit", menuButtonLabelStyle, terminateProgram);
    ...
    public static Menu EditGameMenu(EditorGameDataManager gameData) {
        ...
        for (var visualData : visualEditionDataList) {
            Runnable onClick = () -> menu.addMenuScreen(new EditPanel((EditionData) visualData).getMenuScreen());
            String typeString = "";
            ...
            ActionButton visualSelectButton = ActionButtons.RoundedRectangleButton(2, new Vec2D(300f, 50f), new Vec2D((float) Engine.getNativeWidth() / 2, 900f - (visualListIndex * (50f + 15f))), menuButtonRoundingRadius, menuButtonBorderWidth, Color.menuButtonColor, Color.menuButtonBorderColor, menuButtonLabel, menuButtonLabelStyle, onClick);
