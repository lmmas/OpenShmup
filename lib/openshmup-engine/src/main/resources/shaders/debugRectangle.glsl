...
void main(){
    ...
    vs_out.gQuadSize = a_QuadSize * 2.0f;
    vs_out.gColor = a_Color;
}
...
in vec2 v_InternalCoords;
flat in vec4 v_Color;

out vec4 fragColor;

void main(){
    if(v_InternalCoords[0] < 0.1f || v_InternalCoords[0] > 0.9f || v_InternalCoords[1] < 0.1f || v_InternalCoords[1] > 0.9f){
