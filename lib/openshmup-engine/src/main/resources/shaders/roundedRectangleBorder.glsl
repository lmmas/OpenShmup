...
void main(){
    ...
    v_Out.v_BorderWidth = a_BorderWidth * 2 / a_QuadSize.x;
}
...
void main(){
    float boxSDF = roundedBoxSDF(v_PositionInQuad, v_QuadRelativeHeight, v_RoundingRadius);
    float distance = abs(boxSDF + (v_BorderWidth * 0.5f)) - (v_BorderWidth * 0.5f);
